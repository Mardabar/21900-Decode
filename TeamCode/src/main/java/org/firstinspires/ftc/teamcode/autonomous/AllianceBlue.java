...
@Disabled
@Autonomous(name = "Alliance Blue")
public class AllianceBlue extends OpMode {

    ///  Paths

    private final Pose startPose = new Pose(56, 8, Math.toRadians(90));
    private final Pose preScorePose = new Pose(57, 11, Math.toRadians(112)); // for close shooting do 62, 119, 159
    private final Pose row3Line = new Pose (50, 35.5, Math.toRadians(180));
    private final Pose row3Grab = new Pose (7, 35.5, Math.toRadians(180));
    private final Pose row3Score = new Pose(57, 11, Math.toRadians(112)); // 63, 18, 114
    private final Pose overFlowGrab = new Pose(54.5,47, Math.toRadians(97));
    private final Pose overFlowGrab2 = new Pose(10, 47, Math.toRadians(97));
    private final Pose overFlowScore = new Pose(57, 60, Math.toRadians(112));
    ...
    // Pedro vars
    private PathChain pathPreScore, pathRow1Line, pathRow1Grab, pathRow1Score, pathRow2Line, pathRow2Grab, pathRow2Score, pathRow3Line, pathRow3Grab,  pathRow3Score, pathOverFlowGrab, pathOverFlowScore, pathPark;
    ...
    public void init(){
        ...
        buildPaths();
        setPathState(0);
        telemetry.addData("Current TPS", shooter.flywheel.getVelocity());
        telemetry.update();
    ...
    private void autonomousPathUpdate(){
        switch(pathState){
            ...
            case 6:
                if (!fol.isBusy()){
                    fol.followPath(pathOverFlowGrab);
                    setPathState(7);
                    fol.setMaxPower(.6);
                    shooter.RunBelt(.35);
                } break;
            ...
            case 28:
                if (!fol.isBusy()){
                    ...
                    shooter.feeder.setPosition(openPos);
                        if (beltTimer.milliseconds() > 5000) {
                            shooter.feeder.setPosition(closePos);
                                if (beltTimer.milliseconds() > 5600) {
                                    shooter.stopBelt();
                                    setPathState(8);
                                }
                        }
    ...
    private void buildPaths(){
        ...
        pathOverFlowGrab = fol.pathBuilder()
                .addPath(new BezierLine(row3Score, overFlowGrab))
                .setLinearHeadingInterpolation(row3Score.getHeading(), overFlowGrab.getHeading())
                .build();

        pathOverFlowScore = fol.pathBuilder()
                .addPath(new BezierLine(overFlowGrab, overFlowScore))
                .setLinearHeadingInterpolation(overFlowGrab.getHeading(), overFlowScore.getHeading())
                .build();
    ...
    private void shootFar(int nextState) {
        ...
        if (shootTimer.milliseconds() > 2700) {
            shooter.feeder.setPosition(FeedBackShootSystem.closePos);
        } else {
            shooter.feeder.setPosition(openPos);
        }
